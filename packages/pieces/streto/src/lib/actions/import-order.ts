    ...
    if (stretoProducts.length > 0) {
      ...
      if (stretoProduct !== undefined) {
        orderItems.push({
          ...
          qtyInvoiced: 1, //expected changes soon
          qtyShipped: 1, //expected changes soon
          shippable: true,
          taxes: [],
          details,
          totals: {
...
const getOrderMapping = (
  ...
  return {
    ...
    totals: {
      ...
      shipping: shipping.base_cost + shipping.shipping_option?.cost,
      globalTax: 0,
      tax: order[0].taxes.amount,
      promotionsDiscount: 0,
      couponsDiscount: order[0].coupon?.amount,
      globalDiscount: 0,
